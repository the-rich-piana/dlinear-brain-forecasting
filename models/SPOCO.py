...
class SPOCO(nn.Module):
    ...
    def __init__(self, config: NeuralPredictionConfig, input_size):
        ...
        # Covariate encoder - processes time-varying stimulus covariates
        # The output dimension MUST match self.embedding_dim to be compatible with population embedding
        if hasattr(config, 'covariate_dim') and config.covariate_dim > 0:
            self.covariate_encoder = nn.Sequential(
                nn.Linear(config.covariate_dim, self.embedding_dim // 2),
                nn.ReLU(),
    ...
    def forward(self, x_list, unit_indices=None, unit_timestamps=None, covariates=None):
        """
        ...
        # CRITICAL INJECTION: Covariate enrichment of population embeddings
        # If covariates are provided, process them and add them to the population embedding
        if covariates is not None and self.covariate_encoder is not None:
            # covariates has shape [L, B, D_c]. We need one embedding per session instance.
            # First, aggregate the time dimension to get a single vector per instance.
            # [L, B, D_c] -> [B, D_c]
            aggregated_covariates = covariates.mean(dim=0)

            # Process through the encoder to get an embedding.
            # [B, D_c] -> [B, E]
            covariate_embedding = self.covariate_encoder(aggregated_covariates)

            # The same covariate embedding applies to all neurons in a given batch item.
            # We need to reshape and split it to match the structure of the `embed` list.
            # Split by batch size for each session: [B, E] -> list of [b, E]
            covariate_embedding_list = torch.split(covariate_embedding, bsz, dim=0)

            # Add the covariate embedding to the population embedding for each session.
            # This uses broadcasting: `e` is [b, d, E] and `ce` is [b, 1, E].
            enriched_embed = []
            for e, ce in zip(embed, covariate_embedding_list):
                enriched_embed.append(e + ce.unsqueeze(1))
            
            embed = enriched_embed # Overwrite `embed` with the new enriched version
...
class Model(nn.Module):
    """
    Adapter wrapper to integrate SPOCO (Stimulus Population-Conditioned Forecaster) with the existing DLinear framework.
    Maps standard framework interface to SPOCO's interface with stimulus covariate support.
    
    SPOCO Transformation Summary:
    ✅ Class Names Updated: POCO → SPOCO throughout the codebase
    ✅ Covariate Encoder Added: MLP that maps covariates to embedding space
    ✅ Method Signature Updated: forward() now accepts covariates=None parameter
    ✅ Critical Injection Point: Covariates enrich population embeddings before FiLM conditioning
    ✅ Adapter Integration: Model seamlessly handles x_mark_enc from framework
    
    Architecture Flow:
    1. Framework Input: x_mark_enc [B, L, D_c] contains stimulus covariates
    2. Adapter Processing: Reshapes to [L, B, D_c] and passes to SPOCO
    3. Time Aggregation: [L, B, D_c] → [B, D_c] via mean pooling
    4. Covariate Encoding: [B, D_c] → [B, E] through MLP encoder
    5. Population Enrichment: Adds covariate embedding to population embedding
    6. FiLM Conditioning: Enriched embeddings flow through existing α/β mechanism
    
    Key Benefits:
    - Seamless Integration: Uses existing FiLM conditioning infrastructure
    - Backward Compatible: Gracefully handles cases with no covariates
    - Framework Compliant: Standard interface contracts maintained
    - Minimal Disruption: Only enriches embeddings before existing conditioning
    """
    def __init__(self, configs):
        super(Model, self).__init__()
        
        # Create NeuralPredictionConfig from framework configs
        poco_config = NeuralPredictionConfig()
        
        # Map framework arguments to POCO config
        poco_config.seq_length = configs.seq_len + configs.pred_len  # total sequence length
        poco_config.pred_length = configs.pred_len
        poco_config.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        
        # POCO-specific parameters with sensible defaults
        # Can be extended via configs if needed
        poco_config.compression_factor = getattr(configs, 'compression_factor', 16)
        poco_config.conditioning_dim = getattr(configs, 'conditioning_dim', 1024)
        poco_config.decoder_hidden_size = getattr(configs, 'decoder_hidden_size', 128)
        poco_config.decoder_num_layers = getattr(configs, 'decoder_num_layers', 1)
        poco_config.decoder_num_heads = getattr(configs, 'decoder_num_heads', 16)
        poco_config.poyo_num_latents = getattr(configs, 'poyo_num_latents', 8)
        poco_config.poyo_unit_dropout = getattr(configs, 'poyo_unit_dropout', 0)
        poco_config.rotary_attention_tmax = getattr(configs, 'rotary_attention_tmax', 100)
        
        # For Activity dataset, treat as single session with enc_in channels
        # input_size format: [[num_channels]]  
        input_size = [[configs.enc_in]]
        
        # Initialize SPOCO model
        self.spoco = SPOCO(poco_config, input_size)
        self.pred_len = configs.pred_len
        
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
                enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None):
        """
        Framework interface: expects [B, L, D] tensors
        SPOCO interface: expects list of [L, B, D] tensors and optional covariates
        """
        # x_enc shape: [B, seq_len, D]
        batch_size, seq_len, num_features = x_enc.shape
        
        # Reshape for SPOCO: [B, L, D] -> [L, B, D]
        x_poco = x_enc.permute(1, 0, 2)  # [seq_len, batch_size, num_features]
        
        # Package as list for SPOCO (single session)
        x_list = [x_poco]

        # Process and reshape covariates if they exist
        covariates_poco = None
        if x_mark_enc is not None:
            # Reshape covariates: [B, L, D_c] -> [L, B, D_c]
            covariates_poco = x_mark_enc.permute(1, 0, 2)

        # Forward through SPOCO, passing the prepared covariates
        outputs = self.spoco(x_list, covariates=covariates_poco)
        
        # outputs is list of [pred_len, B, D] tensors
        # Convert back to framework format: [B, pred_len, D]
        pred = outputs[0].permute(1, 0, 2)  # [batch_size, pred_len, num_features]
