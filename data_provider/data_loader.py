...
class Dataset_Activity_Stimulus(Dataset_Activity):
    def __init__(self, root_path, flag='train', size=None,
                 features='M', data_path='activity_raw.csv',
                 target='OT', train_only=False,
                 scale=True, timeenc=0, freq='h'
                 ):
        # Initialize parent class to get basic setup
    ...
    def __read_data__(self):
        ...
        with h5py.File(file_path, 'r') as f:
            ...
            # Load covariate matrix (stimulus-specific functionality)
        ...
        # === STIMULUS-SPECIFIC SPLITTING LOGIC USING TIMESERIESPLITTER ===
    ...
    def __getitem__(self, index):
        # Use valid indices from TimeSeriesSplitter for leak-free sampling
        s_begin = self.valid_indices[index]  # Get actual starting point from valid indices
        
        # Rest of sequence extraction logic
        s_end = s_begin + self.seq_len
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        ...
        # Extract covariate sequences aligned to neural sequences (stimulus-specific)
